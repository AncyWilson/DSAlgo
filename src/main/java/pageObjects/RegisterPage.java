...
public class RegisterPage {
	...
	public static WebDriver driver = DriverFactory.getdriver();
	 ...
	 @FindBy(name = "password2")WebElement passwordConfir;
	...
	// Register
			...
			public Boolean getEmptyRequiredPwdConfirmField() {
				boolean isRequired = false;
				LoggerLoad.info("Checking whether username is empty field");
				if (passwordConfir.getText().isBlank()) {
					...
					isRequired = (Boolean) js_pwdConfirm.executeScript("return arguments[0].required;", passwordConfir);
				}
				return isRequired;
			...
			public String getEmptyFieldErrormsgPwdConfirm() {
				return passwordConfir.getAttribute("validationMessage");
			}
			...
			public void enterValidPwdConfirm(DataTable datatable) {
				...
				for (Map<String, String> form : pwdConfirmDetail) {
					...
					passwordConfir.sendKeys(passwordConfirm);
				}
			}
